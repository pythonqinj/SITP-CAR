...
data['YawRate'] = data["Yaw rate"].astype(float)  # 假设偏航率列名为"yaw_rate"
...
for _id in data['ID'].unique():
    lines[_id] = {
        'line': None,
        'text': None,
        'turn_text': None,  # 转向信息的文本
        'yaw_text': None  # 偏航率的文本
    }
...
def init():
    ...
    for key in lines.keys():
        ...
        if lines[key]['yaw_text'] is not None:
            lines[key]['yaw_text'].set_visible(False)  # 隐藏偏航率文本
    return []
...
def update(frame):
    ...
    for _id in list(lines.keys()):
        if _id not in ids_in_frame:
            ...
            if lines[_id]['yaw_text'] is not None:
                lines[_id]['yaw_text'].set_visible(False)  # 隐藏不活动偏航率文本
    ...
    for idx, row in current_data.iterrows():
        ...
        if lines[_id]['yaw_text'] is None:
            lines[_id]['yaw_text'] = ax.text(0, 0, f"Yaw: {row['YawRate']:.2f}", verticalalignment='top', fontsize=5, color='blue', visible=True)
        x, y = row['X'], row['Y']
        ...
        lines[_id]['yaw_text'].set_position((x, y - 2))  # 显示在转向信息下方
        lines[_id]['yaw_text'].set_visible(True)
    return []
